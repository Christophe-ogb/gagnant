...
export function QuizCard({ heritageId, heritageName, challenge, quiz }: Props) {
  ...
  // Le nom saisi par le joueur est celui qui apparaît sur sa carte téléchargée.
  const displayedPlayerName = playerName.trim() || "Joueur Gagnants 229";
  const displayHeritageName = heritageName.replace(/^commune\s+(de\s+)?/i, "").trim();
  ...
  async function downloadRewardCard(score = totalPoints, completed = true) {
    ...
    context.fillText(`+${score} POINTS`, 600, 320);
    context.fillStyle = "#f4f1ea";
    ...
    context.fillText(`Épreuve de ${displayHeritageName}`.slice(0, 54), 600, 438);
    context.fillStyle = "#d4af37";
    context.font = "600 27px sans-serif";
    context.fillText(`👑 ${completed ? `Gardien du Patrimoine de ${displayHeritageName}` : `Explorateur de ${displayHeritageName}`}`.slice(0, 60), 600, 485);
    context.fillStyle = "#d8cfc4";
    ...
    link.download = `gagnants-229-${heritageId}-${completed ? "carte-victoire-v5" : "carte-score-v5"}.png`;
    link.click();
    setDownloaded(true);
